...
from Previous_Prototypes.Prototype.config.config_manager import load_config  # import config loader
...
class RealTimeData:
    ...
    def is_sensor_connected(pin, threshold=50, samples=10):
        ...
        """
        # For pyfirmata2, we'll assume the sensor is connected and let the callback handle it
        # This is a workaround since pyfirmata2 doesn't support polling
        return True
    ...
    def _acquire(self, ch):
        ...
        # Set up callback for pyfirmata2
        def callback(data):
            nonlocal index
            val = int(data * 1023) if data is not None else 0  # Convert to integer
