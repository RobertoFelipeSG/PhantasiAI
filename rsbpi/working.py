...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def handle_data_update(self, times, values):
        """Handle new data from the data thread"""
        if hasattr(self, 'recorder') and self.recorder.recording:
            # Get the latest timestamp and value
            latest_time = times[-1]
            latest_value = values[-1, 0] if values.ndim > 1 else values[-1]
            self.recorder.record_data_point(latest_time, latest_value)
