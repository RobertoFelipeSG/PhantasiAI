...
def run_bayesian_optimization(resp_nd, resp_1d, parameter_names, X_test_norm, X_test, bounds_discrete,
    ...
    for s in range(n_suj):
        ...
        if 'Freq' in best_params_values:
            client.publish('GPBO/frequence', str(best_params_values['Freq']))
        if 'Amp' in best_params_values:
            client.publish('GPBO/amplitude', str(best_params_values['Amp']))
        if 'DutyCycle' in best_params_values:
            client.publish('GPBO/dutycycle', str(best_params_values['DutyCycle']))
