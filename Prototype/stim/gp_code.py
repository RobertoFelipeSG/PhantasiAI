...
from detect_change import latest_subdir

# Fonction pour lire les profils des sujets
def read_subject_profiles(file_name):
    data = pd.read_csv(file_name, sep=';')
    subject_profiles = []
    parameter_names = [col for col in data.columns if col not in ['Subject', 'Interaction_Scale', 'Interaction_Sign']]
    n_dim = len(parameter_names)
    n_val_list = []
    for idx, row in data.iterrows(): #NOTE TO SELF: IGNORE THE FIRST ROW!
        subject = {}
        n_val_subject = None
        print(row)
        for param in parameter_names:
            ...
            if n_val_subject is None:
                ...
                if len(values) != n_val_subject:
                    print(f"Ignoring the 1st incomplete row. The number of values from the subject is not coherent")
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(latest_subdir, 'peak_classificat.txt')
    subject_profiles, parameter_names, n_dim, n_val = read_subject_profiles(data_path)#'./../emg/emg-recordings/peak_classificat.txt')
