...
[ Python Server (main.py) ]  <-- Handles all object/thread/flag creation and controls uvicorn application
       |
[ Ganglion (data_acquisition.py) ]  <-- Catches and processes the raw data from OpenBCI Ganglion device
       |
       +----------------------------------------------+
       |                                              |
  [ Path 1: Visualization ]                [ Path 2: Data Processing (recorder.py and emg_feature_extractor.py)]
       |                                              | 
  [ WebSockets (connection_manager.py)]    [ DataFrames/CSV Files (features.txt)]
       |                                              |
       v                                              v
  [ Browser ]                              [ Local RPi Device ]
(Live EMG data/real-time instructions)                |
  (frontend/)                              [ Path 3: Optimization (change_detector.py and gpbo.py)]
                                             -> or calb_change_detector.py and calibrator.py
                                                      |
                                                      v
                                           [ Electrical stimulation via GPIO (square.py) ] 
```
...
7. Saving profiler log as csv: separate thread to call session.profiler.save_as_csv(Path)
8. Stopping calibrator/optimizer: separate thread to stop calibrator/optimizer and save session data via session.calibrator/optimizer.handle_stop()
