...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, mode, mode_label="", data_type="database"):
        super().__init__()
        self.mode = mode                        # 'pro' or 'chat'
        self.data_type = data_type              # 'live' or 'database'
        self.selected_channels = []             # Indices of user-selected EMG channels
        ...
        # Recorder initialized once to track recording state and write to CSV
        self.recorder = EMGRecorder(self)

        # Choose between live Arduino stream or offline file
    ...
    def _build_ui(self):
        ...
        # Main horizontal layout: Graph + Chat
        self.body_layout = QtWidgets.QHBoxLayout()
        self.main_layout.addLayout(self.body_layout)

        # Add chat widget (event log panel)
    ...
    def _init_live(self):
        ...
        # Default to first channel if live mode
        self.selected_channels = [0]
        self.setup_live_graph()

    def load_npz_file(self):
        # Load EMG data from file (NumPy archive)
    ...
    def build_database_view(self):
        # Rebuild layout and graph for offline (npz) data
        ...
        # Connect signals for chat + recording
    ...
    def setup_live_graph(self):
        # Reset layout and launch real-time plot from Arduino
        for i in reversed(range(self.body_layout.count())):
            w = self.body_layout.itemAt(i).widget()
            if w:
                w.setParent(None)
    ...
    def show_channel_dialog(self):
        # Allow user to select channels to view/record
    ...
    def handle_data_update(self, times, values):
        # Receive data from GraphWidget's thread and log to recorder
        if self.recorder.recording:
            latest_time = times[-1]
    ...
    def keyPressEvent(self, event):
        # Press 'M' to mark an event timestamp
        if event.key() == QtCore.Qt.Key_M:
            self.recorder.mark_event()
        super().keyPressEvent(event)

    def save_logs(self):
        # Save chat/event log as text
    ...
    def closeEvent(self, event):
        # Clean shutdown: stop threads and recorder
