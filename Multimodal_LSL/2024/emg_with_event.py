...
class DataThread(QtCore.QObject):
    ...
    def __init__(self, analog_input=None, data=None, channel=None, seconds_to_display=1):
        ...
        # updated csv header
        with open("emg_log_with_events.csv", "w") as file:
            file.write("EMG,Timestamp,Event\n")
    ...
    # updated log data with event flag
    def log_data(self, timestamp, emg):
        event_flag = 1 if round(timestamp, 3) in event_timestamps else 0
        with open("emg_log_with_events.csv", "a") as file:
            file.write(f"{emg},{timestamp:.6f},{event_flag}\n")
